 ...
 * onnxruntime-web (WASM), and writes the chosen stem back to the editor
...
(function ( w, d, PKAE ) {
	...
	function separate ( mix, stemRow, onProgress ) {
		var ort = w.ort;
		var totalLen = mix[0].length;
		var nChunks = Math.max(1, Math.ceil(totalLen / STRIDE));
		var out = [new Float32Array(totalLen), new Float32Array(totalLen)];
		...
		var rowOffset = (stemRow * 2) * N_SAMPLES;
			...
			return session.run({ mix: inputTensor }).then(function (result) {
				...
				for (var c2 = 0; c2 < 2; ++c2) {
					...
					for (var s2 = 0; s2 < chunkLen; ++s2)
						out[c2][start + s2] += stems[rowStart + s2] * win[s2];
				}
	...
	function runSeparation ( stemId, ui ) {
		var eng = engine();
		...
		getSession(function (phase, frac, received) {
			...
			if (cancelled) throw new Error('Cancelled');
			var row = stemId === 'accompaniment' ? STEM_ROW.vocals : STEM_ROW[stemId];
			return separate(mix, row, function (frac, i, n) {
				ui.progress(frac, 'Running AI model: chunk ' + i + '/' + n + '...');
			}).then(function (stem) {
				if (stemId === 'accompaniment') return subtractStem(mix, stem);
				return stem;
			});
		}).then(function (stem) {
			if (cancelled) throw new Error('Cancelled');
			ui.progress(1, 'Finalizing...');
			return stemToBuffer(stem, buffer.sampleRate, buffer.numberOfChannels);
		}).then(function (outBuf) {
			var label = stemId.charAt(0).toUpperCase() + stemId.slice(1);
			...
			var h = host();
			if (region) h.Replace(start, dur, outBuf);
			else h.FullReplace(outBuf);
	...
	app.listenFor('RequestActionFXUI_StemAI', function () {
		...
		var body = '<div class="pk_row"><label>Stem to extract</label>';
		for (var i = 0; i < STEM_LIST.length; ++i) {
			body += '<input type="radio" class="pk_check" id="pkai' + i + '" name="pkaistem" value="' +
		...
		body += '</div>' +
			...
			'Separation is slow on CPU — a few minutes per song. Applies to the selection, or the whole file.</div>' +
